...
if ~isfield(session,'extracellular') && ~isfield(session.extracellular,'chanCoords') 
    ...
    %session.extracellular.chanCoords.verticalSpacing = 200; % (µm) Vertical spacing between sites.
end
...
if ~isempty(rezFile)
    ...
    %if the electrode group indicated by kilosort should be overridden
